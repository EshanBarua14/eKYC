"""M60: Add edd_cases and edd_actions — BFIU §4.2/§4.3
Revision ID: m60_edd_workflow
Revises: 20260424_205733
...
def upgrade():
    ...
    op.execute("""
        ...
        BEGIN
            IF TG_OP = 'UPDATE' OR TG_OP = 'DELETE' THEN
                RAISE EXCEPTION 'edd_actions is append-only. % not permitted. BFIU §5.1', TG_OP;
            END IF;
            RETURN NEW;
        END;
        $$ LANGUAGE plpgsql;
    ...
    op.execute("""
        ...
        RETURNS TRIGGER AS $$
        BEGIN NEW.updated_at = NOW() AT TIME ZONE 'Asia/Dhaka'; RETURN NEW; END;
        $$ LANGUAGE plpgsql;
    """)
    op.execute("""
